...
class Sender
{
    ...
    public function send(Message $message)
    {
        ...
        if (!$this->serverApiKey) {
            throw new myException("Server API Key not set", Exception::ILLEGAL_API_KEY);
        }

        //GCM response: Number of messages on bulk (1001) exceeds maximum allowed (1000)
        if (count($message->getRegistrationIds()) > 1000) {
            throw new myException("Malformed request: Registration Ids exceed the GCM imposed limit of 1000", myException::MALFORMED_REQUEST);
        }
        ...
        switch ($resultHttpCode) {
            ...
            default:
                echo "unknown error";
                //TODO: Retry-after
                throw new myException("Unknown error. ".$resultBody, myException::UNKNOWN_ERROR);
                break;
        }
